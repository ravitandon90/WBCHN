...
filePath = '..\Sensor_Network\Sensor_Network_';
fileName = strcat (filePath, int2str(round(sqrt(Area_Net))), '_', int2str(Num_Sensors),'.txt');

while (count < maxCount)    
    Radius_Net = Side_Net / sqrt(pi);
    filePath = '..\Sensor_Network\Sensor_Network_';
    fileName = strcat (filePath, int2str(Side_Net), '_', int2str(Num_Sensors),'.txt');
    current_Cluster_Radius = Side_Net / 4;
...
Side_Net = Side_Net + Side_Net_Step;
end
